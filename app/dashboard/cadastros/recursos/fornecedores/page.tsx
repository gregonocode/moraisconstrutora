...
export default async function FornecedoresPage({ searchParams }: PageProps) {
  ...
  const [fornecedoresRes, totalRes, ativosRes, inativosRes] = await Promise.all(
    [
      getFornecedores({
        supabase,
        userId,
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-relaxed text-white/60 sm:mt-4 sm:text-base">
              Gerencie os fornecedores da operação com contatos, documento,
              razão social e status de ativação.
            </p>
          </div>
      ...
      <section className="relative overflow-hidden rounded-[20px] border border-white/5 bg-[#252525] p-4 shadow-lg sm:rounded-[28px] sm:p-5 lg:p-6">
        ...
        <div className="relative z-10">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-white/50">
                Busque, filtre e gerencie os fornecedores cadastrados.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <Link
                href="/dashboard/cadastros/recursos/fornecedores/novo"
          ...
          >
            <div className="flex items-center gap-3 rounded-2xl border border-white/5 bg-white/[0.04] px-4 py-3">
              ...
              <input
                ...
                defaultValue={q}
                type="text"
                placeholder="Buscar por nome, razão social, documento ou contato"
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="truncate text-xs text-white/45">
                                {item.razao_social?.trim() || "Sem razão social"}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 text-sm text-white/65">
                          <div className="space-y-1">
                            <p>{item.contato_nome || "-"}</p>
                            <p className="inline-flex items-center gap-1 text-white/45">
          ...
          <div className="mt-4 flex flex-col gap-3 text-sm text-white/45 sm:flex-row sm:items-center sm:justify-between">
            <p>Mostrando {fornecedores.length} fornecedores</p>
...
async function getFornecedores({
  ...
  let query = supabase
    .from("fornecedores")
    .select(
      "id, nome, razao_social, cpf_cnpj, telefone, email, contato_nome, ativo"
    )
    .eq("user_id", userId)
  ...
  if (q) {
    query = query.or(
      `nome.ilike.%${q}%,razao_social.ilike.%${q}%,cpf_cnpj.ilike.%${q}%,contato_nome.ilike.%${q}%`
    );
  }
